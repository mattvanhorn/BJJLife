...
Then /^I should see the product I added$/ do
  Site.products_admin_page.products.detect{|p|p.has_text? Product.last.name}.should be_present
end

Then /^I should see the category I added$/ do
  Site.categories_admin_page.categories.detect{|c|c.has_text? Category.last.name}.should be_present
end

Then(/^I should see all the categories$/) do
  Category.all.each do |cat|
    Site.products_page.categories.detect{|c|c.has_text? cat.name}.should be_present
  end
end

Then(/^I should see all the products$/) do
  Product.all.each do |prod|
    Site.products_page.products.detect{|p|p.has_text? prod.name}.should be_present
  end
end
